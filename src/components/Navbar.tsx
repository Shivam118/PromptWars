...
export const Navbar = () => {
  ...
  return (
    <nav aria-label="Main Navigation" className="sticky top-0 z-50 w-full border-b border-white/10 bg-black/50 backdrop-blur-md">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
            ...
            <div className="ml-10 flex items-baseline space-x-8" role="menubar">
              {links.map((link) => {
                ...
                return isDisabled ? (
                  ...
                  >
                    {link.name}
                    <span className="absolute -top-3 -right-3 whitespace-nowrap rounded-full bg-white/10 px-1.5 py-0.5 text-[10px] text-gray-400 opacity-0 transition-opacity group-hover:opacity-100" aria-hidden="true">
                      Soon
                    </span>
                  </button>
                ) : (
          ...
          <div className="flex items-center">
            <Link
              href="/admin"
              aria-label="Navigate to Admin Control Center"
